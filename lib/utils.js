...
exports.matchRoutes = function (req, res) {
  ...
  let version = getVersion(path, 1);

  res.render(path, function (err, html) {
    if (err) {
      res.render(path + '/index', function (err2, html) {
        if (err2) {
          const basePath = handleMissingVersionRoute(version, `${path}`, res);
          res.render(basePath, function (err3, html3) {
            if (err3) {
              res.status(404).send(err + '<br>' + err2 + '<br>' + err3)
            } else {
              res.end(html3)
            }
          });
